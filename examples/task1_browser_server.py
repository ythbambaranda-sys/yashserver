...
mongo_client: yashserver.DatabaseClient | None = None
...
def _connect_mongo(*, force: bool = False) -> bool:
    ...
    try:
        mongo_client = yashserver.connect_database(
            "mongodb",
            uri=MONGO_URI,
            database=MONGO_DB,
        ...
        mongo_gridfs_error = ""
        mongo_last_error = ""
        mongo_last_connected_at = yashserver.ServerTools.utc_now()
        return True
...
http_server = yashserver.YSyncHttpServer(
    host="127.0.0.1",
    port=8080,
...
ws_server = yashserver.YSyncWebSocketServer(
    host="127.0.0.1",
    port=9001,
    auth_token=AUTH_TOKEN if AUTH_ENABLED else None,
...
log_plugin = yashserver.LoggingPlugin()
...
def _save_text_message_to_mongo(
    *,
    session: yashserver.WebSocketClient,
    sender: str,
    text: str,
    source: str,
) -> None:
    ...
    document = {
        ...
        "created_at": yashserver.ServerTools.utc_now(),
    }
    ...
    for attempt in range(2):
        try:
            ...
            mongo_last_saved_at = yashserver.ServerTools.utc_now()
            mongo_last_error = ""
            return
        except Exception as error:
            mongo_write_errors += 1
...
async def _broadcast_completed_transfer(transfer: IncomingTransfer, server: yashserver.YSyncWebSocketServer) -> None:
    assembled = bytearray()
...
async def _handle_binary_chunk(
    session: yashserver.WebSocketClient,
    payload: bytes,
    server: yashserver.YSyncWebSocketServer,
) -> dict[str, Any] | None:
...
def home(_request, _server):
    return yashserver.YSyncHttpServer.html(load_page_html(), headers={"Cache-Control": "no-store"})
...
def stats(_request, _server):
    ...
    return yashserver.YSyncHttpServer.json(snapshot)
...
def mongo_recent(request, _server):
    if not MONGO_TEXT_ENABLED:
        return yashserver.YSyncHttpServer.json({"error": "mongo-text-disabled"}, status=503)
    if not _connect_mongo():
        return yashserver.YSyncHttpServer.json({"error": "mongo-unavailable", "detail": mongo_connect_error}, status=503)
    ...
    try:
        ...
        return yashserver.YSyncHttpServer.json({"error": "mongo-query-failed", "detail": str(error)}, status=500)
...
def mongo_recent_files(request, _server):
    ...
    try:
        ...
        gridfs_documents = mongo_client.find(
            ...
            {"metadata.app": "yashserver", "metadata.type": "file"},
            limit=limit,
            sort=[("uploadDate", -1)],
        )
...
def chat(
    ...
    payload = yashserver.ServerTools.from_json(message, default=None)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if (
            ...
            print("Port already in use. Stop the previous yashserver process, then run again.")
