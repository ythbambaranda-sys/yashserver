...
class TestWebSocketServer(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        self.server = yashserver.YWebSocketServer(host="127.0.0.1", port=0, rate_limit_per_window=2, rate_limit_window_seconds=60)
    ...
    async def test_rate_limit_returns_error_payload(self) -> None:
        ...
        self.assertEqual(third.get("message"), DDOS_BLOCK_MESSAGE)
        writer.close()
        await writer.wait_closed()

    async def test_setddosprot_disables_rate_limit(self) -> None:
        self.server.setddosprot(False)
        reader, writer = await self._connect()
        for _ in range(3):
            writer.write(_make_client_frame(0x1, b'{"text":"hello"}', fin=True))
            await writer.drain()
        ...
        for _ in range(3):
            _opcode, payload = await _read_server_frame(reader)
    ...
    async def test_auth_rejects_missing_token(self) -> None:
        await self.server.stop()
        self.server = yashserver.YWebSocketServer(host="127.0.0.1", port=0, auth_token="secret-token")
