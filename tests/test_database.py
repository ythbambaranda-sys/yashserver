...
class TestDatabaseSupport(unittest.TestCase):
    def test_supported_database_list_contains_requested_engines(self) -> None:
        ...
        supported = {name.lower() for name in yashserver.list_supported_databases()}
    ...
    def test_unknown_backend_raises(self) -> None:
        with self.assertRaises(yashserver.UnsupportedDatabaseError):
            yashserver.connect_database("not-real-db")
    ...
    def test_missing_dependency_error_is_clear(self) -> None:
        ...
        with mock.patch("yashserver.database.importlib.import_module", side_effect=fake_import):
            with self.assertRaises(yashserver.MissingDependencyError):
                yashserver.connect_database("mongodb", uri="mongodb://127.0.0.1:27017", database="test")

    def test_server_tools_database_helpers(self) -> None:
        supported = yashserver.ServerTools.supported_databases()
        self.assertIn("SQLite", supported)
        db = yashserver.ServerTools.connect_database("sqlite", database=":memory:")
        try:
            self.assertTrue(db.ping())
        finally:
            db.close()
